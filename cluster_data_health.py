...
file_path = './datasets/CVD_cleaned_processing_3.csv'  # 请替换为您的CSV文件路径
data = pd.read_csv(file_path)

#choose the correct column and Sex
selected_data = data[['Checkup_New', 'exercising_New', 'Alcohol_Consumption_New', 'Smoking_History_New', 'Green_Vegetables_Consumption', 'FriedPotato_Consumption_New','Fruit_Consumption', 'Sex']]
...
#save the label
data.loc[data['Sex'] == 'Male', 'Cluster_Labels_health_male'] = male_cluster_labels
data.loc[data['Sex'] == 'Female', 'Cluster_Labels_health_female'] = female_cluster_labels

output_file_path_male = './cluster_label_male_3.csv'
data[data['Sex'] == 'Male'][['Cluster_Labels_health_male']].to_csv(output_file_path_male, index=False)

output_file_path_female = './cluster_label_female_3.csv'
data[data['Sex'] == 'Female'][['Cluster_Labels_health_female']].to_csv(output_file_path_female, index=False)
...
ax1.set_title('Clustering_result_health- Male',pad = 5)
...
ax2.set_title('lustering_result_health- Female',pad = 4)
ax2.legend(*scatter2.legend_elements(), title='Clusters')

plt.tight_layout()
plt.savefig('./results_picture/clustering_result_by_gender_health.png')
plt.show()
